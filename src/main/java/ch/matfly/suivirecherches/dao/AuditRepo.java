	...
	public List<RechercheAuditDto> getRechercheAuditRevForId(Long id){
		...
		return jdbcTemplate.query(
				"SELECT c.id, c.contact_date, c.poste, c.statut, c.assignationORP, " +
						...
						"WHERE id=?",new Object[] {id},
						(rs, rowNum) ->
				RechercheAuditDto.of(rs.getLong(1), MatFormat.format(rs.getDate(2)), rs.getString(3), rs.getString(4), rs.getString(5),
						rs.getString(6), rs.getString(7), rs.getLong(8), rs.getLong(9), rs.getLong(10),
                        rs.getLong(11), rs.getInt(12),rs.getInt(13),MatFormat.format(rs.getLong(14))));
	}
}